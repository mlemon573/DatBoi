...
public class Canvas extends JPanel implements ModelListener, Serializable {
		...
		if (selected != null) {
			for (Point p : selected.getKnobs()) {
				if (p != null) {
					...
					g.fillRect((int) p.getX(), (int) p.getY(), DShape.KNOB_SIZE, DShape.KNOB_SIZE);

					g.setColor(Color.BLACK);
					g.drawRect((int) p.getX(), (int) p.getY(), DShape.KNOB_SIZE, DShape.KNOB_SIZE);
				}
			}
		}
	}
	 ...
	 * 
	 * @param whiteboard
	 *            - the whiteboard to invoke.
	 */
	void startClient(Whiteboard whiteboard) {
		String result = JOptionPane.showInputDialog("Connect to host:port", default_host + ":" + default_port);
		if (result != null) {
			mode = "Client";
	...
	public Point findKnob(int x, int y) {
		for (Point knob : selected.getKnobs()) {
			if (knob != null && x >= knob.getX() && x <= knob.getX() + DShape.KNOB_SIZE && y >= knob.getY()
					&& y <= knob.getY() + DShape.KNOB_SIZE) {
				return knob;
			}
		}
		return null;
	 ...
	 * 
	 * @param point
	 *            - a point which represents a knob.
	 */
	 ...
	 * 
	 * @param i
	 *            - the numerical value that represents a specific knob.
	 */
	 ...
	 * 
	 * @param dx
	 *            - the amount the x value has changed.
	 * @param dy
	 *            - the amount the y value has changed.
	 */
	 ...
	 * 
	 * @param id
	 *            - the unique numerical value of a shape.
	 ...
	 * 
	 * @param model
	 *            - the model who has been changed.
	 */
	@Override
